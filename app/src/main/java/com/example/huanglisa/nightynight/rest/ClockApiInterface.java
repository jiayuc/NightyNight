...
public interface ClockApiInterface {
    ...
    @FormUrlEncoded
    @PUT("clock/{id}")
    Call<ReceivedClock> updateClock(@Header("x-zumo-auth") String authorization, @Field("sleepHour") int sleepHour, @Field("sleepMin") int sleepMin, @Field("wakeHour") int wakeHour, @Field("wakeMin") int wakeMin, @Path("id") String id);
